...
class CasesCsv:
    ...
    def csv(self):
        ...
        color = ['blue', 'red', 'green', 'yellow', 'orange','purple', 'pink', 'brown', 'grey']
        for pais in ['world', 'brazil', 'spain', 'us', 'italy']:
            with open(f'/home/semantix/PycharmProjects/Desafio4/{pais}.csv','r') as csvfile:
                ...
                for row in readCSV:
                    if row[0] != 'Country':
                        if pais == 'world':
                            wld.append(row[1].replace('.', ''))
                        elif pais == 'brazil':
                            br.append(row[1].replace('.',''))
                            dt.append(row[4])
                        elif pais == 'spain':
                            sp.append(row[1].replace('.', ''))
                        elif pais == 'us':
                            us.append(row[1].replace('.', ''))
                        else:
                            it.append(row[1].replace('.', ''))
                csvfile.close()
        ...
        if ((int(wld[-1]) / 1000000) -  int(int(wld[-1]) / 1000000)) >= 0.5:
            for wz in range(1, (int(int(wld[-1]) / 1000000) + 2)):
                corona_cases['wld_lbl'].append(wz*1)
                corona_cases['wld_ylbl_loc'].append(420 - wz * 1000000 * (358 / int(wld[-1])))
        else:
            for wz in range(1, (int(int(wld[-1]) / 1000000) + 2)):
                corona_cases['wld_lbl'].append(wz*1)
                corona_cases['wld_ylbl_loc'].append(420 - wz * 1000000 * (358 / int(wld[-1])))
        if ((int(ymax) / 200000) - int(int(ymax) / 200000)) >= 0.5:
            for cz in range(1, (int(int(ymax) / 200000) + 2)):
                corona_cases['ctry_lbl'].append(cz*200)
                corona_cases['ctry_ylbl_loc'].append(420 - cz * 200000 * (358 / int(ymax)))
        else:
