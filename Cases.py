...
class CasesCsv:
    ...
    def csv(self):
        ...
        if ((int(cases['wmax'][0]) / 1000000) -  (int(int(cases['wmax'][0]) / 1000000))) >= 0.5:
            for wz in range(0, (int(int(cases['wmax'][0]) / 1000000)) + 2):
                cases_svg['wld_lbl'].append(wz*1)
                cases_svg['wld_ylbl_loc'].append(420 - wz * 1000000 * (358 / int(cases['wmax'][0])))
        else:
            for wz in range(0, (int(int(cases['wmax'][0]) / 1000000) + 1)):
                cases_svg['wld_lbl'].append(wz*1)
                cases_svg['wld_ylbl_loc'].append(420 - wz * 1000000 * (358 / int(cases['wmax'][0])))

        if ((int(ymax) / 200000) - int(int(ymax) / 200000)) >= 0.5:
            for cz in range(0, (int(int(ymax) / 200000) + 2)):
                cases_svg['ctry_lbl'].append(cz*200)
                cases_svg['ctry_ylbl_loc'].append(420 - cz * 200000 * (358 / int(ymax)))
        else:
